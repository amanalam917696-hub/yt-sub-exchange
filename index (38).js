...
const paymentSchema = new mongoose.Schema({
  ...
  userName: String,
  plan: String,
  type: { type: String, default: 'plan' }, // 'plan' = old YT plan purchase, 'topup' = wallet add money
  amount: Number,
...
// ============================================================
// WALLET TOP-UP (real money via Cashfree — CSC system)
// ============================================================
app.post('/api/wallet/create-order', async (req, res) => {
  try {
    const { userId, amount } = req.body;
    const amt = parseFloat(amount);
    if (!amt || amt < 10) return res.json({ success: false, message: 'Minimum ₹10 top-up karo!' });
    ...
    const orderData = {
      ...
      order_meta: {
        return_url: 'https://ytsubexchange.online/payment-success?order_id={order_id}&type=topup',
        notify_url: 'https://yt-sub-exchange.onrender.com/api/cashfree/webhook'
      }
    };
    ...
    const payment = new Payment({
      userId, type: 'topup', amount: amt, transactionId: orderId,
      status: 'pending', userName: user.name, userEmail: user.email
    });
    await payment.save();

    await sendTelegram(
      `💳 WALLET TOP-UP STARTED!\n\n` +
      `👤 ${user.name || 'Unknown'}\n` +
      `📧 ${user.email}\n` +
      `💰 Amount: ₹${amt}\n` +
      `🔖 Order: ${orderId}\n` +
    ...
    console.error('Wallet topup create order error:', e.message);
    res.json({ success: false, message: e.message });
  }
});
...
app.post('/api/wallet/verify', async (req, res) => {
  try {
    ...
    const payment = await Payment.findOne({ transactionId: orderId, type: 'topup' });
    if (!payment) return res.json({ success: false, message: 'Order nahi mila' });

    if (payment.status === 'completed') {
      const user = await User.findById(userId);
      return res.json({ success: true, message: 'Wallet pehle hi credit ho chuka!', newBalance: user?.walletBalance || 0 });
    }
    ...
    const orderStatus = response.order_status;
    console.log(`Wallet verify: orderId=${orderId}, status=${orderStatus}`);
    ...
    if (orderStatus === 'PAID') {
      ...
      if (user) {
        ...
        res.json({ success: true, message: 'Wallet mein paisa aa gaya!', newBalance: user.walletBalance });
      } else {
