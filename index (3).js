...
app.post('/api/payment/verify', async (req, res) => {
  try {
    const { paymentId, transactionId, userId, plan } = req.body;
    ...
    if(payment) {
      ...
      // Manual verification - find user by userId and update plan
      const user = await User.findById(userId);
      if(!user) return res.json({ success: false, message: 'User nahi mila' });
      
      // Save payment record
      const newPayment = new Payment({ 
        userId, plan, amount: req.body.amount || 0, 
      ...
      return res.json({ success: false, message: 'Payment verify nahi hua' });
    }
