...
// ============================================================
const GMAIL_USER = process.env.GMAIL_USER || 'amanalam917696@gmail.com';
const GMAIL_APP_PASSWORD = process.env.GMAIL_APP_PASSWORD || 'noxheecbqeyzcwfk';
...
const mailTransporterFallback = nodemailer.createTransport({
  host: 'smtp.gmail.com',
  port: 587,
  secure: false,
  auth: {
...
async function sendOtpEmail(toEmail, otp) {
  const mailOptions = {
    from: `"YT Sub Exchange" <${GMAIL_USER}>`,
    to: toEmail,
    subject: `${otp} — Aapka OTP Code`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:420px;margin:0 auto;padding:30px 20px;background:#fff">
        <h2 style="color:#ff0000;margin-bottom:4px">YT Sub Exchange</h2>
        <p style="color:#555;font-size:14px">Aapka login OTP code hai:</p>
        <div style="background:#fff3f3;border:2px dashed #ff0000;border-radius:12px;padding:18px;text-align:center;margin:18px 0">
          <span style="font-size:32px;font-weight:900;letter-spacing:6px;color:#ff0000">${otp}</span>
        </div>
        <p style="color:#888;font-size:12.5px">Ye code 10 minute ke liye valid hai. Kisi ke saath share na karein.</p>
      </div>
    `
  };
  ...
  try {
    ...
    console.log(`📧 OTP email sent to ${toEmail} (port 465)`);
    return true;
  } catch(e) {
    console.error('Email send error (465):', e.message);
    // Fallback to port 587
    try {
      await mailTransporterFallback.sendMail(mailOptions);
      console.log(`📧 OTP email sent to ${toEmail} (port 587 fallback)`);
      return true;
    } catch(e2) {
      console.error('Email send error (587 fallback):', e2.message);
      return false;
    }
  }
}
...
app.post('/api/otp/send', async (req, res) => {
  try {
    ...
    if (!email || !email.includes('@')) {
      return res.json({ success: false, message: 'Valid email daalo!' });
    }
    ...
    // Purane unverified OTPs hatao isi email ke
    await Otp.deleteMany({ email: cleanEmail, verified: false });

    await new Otp({ email: cleanEmail, otp, expiresAt }).save();

    const sent = await sendOtpEmail(cleanEmail, otp);
    if (!sent) {
      return res.json({ success: false, message: 'Email bhejne mein error aaya, dobara try karo!' });
    }

    res.json({ success: true, message: 'OTP bhej diya! Email check karo (Spam folder bhi).' });
  } catch(e) {
...
app.post('/api/otp/verify', async (req, res) => {
  try {
    ...
    if (!email || !otp) return res.json({ success: false, message: 'Email aur OTP daalo!' });
    const cleanEmail = email.trim().toLowerCase();

    const record = await Otp.findOne({ email: cleanEmail, otp: otp.toString(), verified: false }).sort({ createdAt: -1 });
    if (!record) return res.json({ success: false, message: 'Galat OTP!' });
    if (new Date() > record.expiresAt) return res.json({ success: false, message: 'OTP expire ho gaya, naya mangao!' });
    ...
    if (!user) {
      ...
      user = new User({ email: cleanEmail, name: cleanEmail.split('@')[0], referredBy: refCode });
      await user.save();
      isNewUser = true;
      await ensureReferralCode(user);

      await sendTelegram(
        `🆕 NEW USER JOIN! (Email OTP)\n\n` +
        `👤 Name: ${user.name}\n` +
        `📧 Email: ${user.email}\n` +
        (refCode ? `🔗 Referred by: ${refCode}\n` : '') +
        `🕐 Time: ${new Date().toLocaleString('en-IN', {timeZone:'Asia/Kolkata'})}`
