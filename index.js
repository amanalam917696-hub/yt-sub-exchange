...
const sessionStore = MongoStore.create({
  ...
  ttl: 365 * 24 * 60 * 60 // session store bhi 1 saal tak rakhega (cookie maxAge se match)
});
sessionStore.on('error', (err) => {
  console.error('❌ SESSION STORE ERROR (MongoDB):', err.message);
});
...
app.use(session({
  ...
  rolling: true, // har request pe expiry refresh — jab tak use karte raho, kabhi expire nahi hoga
  store: sessionStore,
  cookie: {
    secure: true,
    sameSite: 'none',
    domain: '.ambds.com',
    maxAge: 365 * 24 * 60 * 60 * 1000 // 1 saal — practically "kabhi logout nahi"
  }
}));
...
const serviceLinkSchema = new mongoose.Schema({
  key: { type: String, unique: true },   // jaise 'jamabandi', 'pan-card', 'aadhaar'
  name: { type: String, default: '' },   // display ke liye
  url: { type: String, default: '' },    // real destination link — khali ho to "Jald aayega"
...
const demoTradeSchema = new mongoose.Schema({
  userId: mongoose.Schema.Types.ObjectId,
  symbol: String,        // jaise 'bitcoin', 'ethereum'
  quantity: Number,       // kitna coin khareeda (virtual)
  entryPrice: Number,     // ₹ mein
  exitPrice: Number,
  amountInr: Number,      // kitna virtual paisa lagaya
  status: { type: String, default: 'open' }, // open / closed
  pnl: { type: Number, default: 0 },
  openedAt: { type: Date, default: Date.now },
  closedAt: Date
});
const DemoTrade = mongoose.model('DemoTrade', demoTradeSchema);
...
const userSchema = new mongoose.Schema({
  ...
  demoBalance: { type: Number, default: 10000 }, // Virtual paisa — sirf practice ke liye, kabhi real nahi
  referralCredits: { type: Number, default: 0 },       // ₹7 per referral — agla membership renewal ke kaam aayega
  firstMembershipPaid: { type: Boolean, default: false }, // referral bonus sirf pehli baar milta hai
  // ===== REFERRAL SYSTEM =====
...
// ✅ AMBDS — Referral se ₹7 credit (sirf pehli membership payment pe)
const REFERRAL_MEMBERSHIP_BONUS = 7;
async function creditMembershipReferral(paidUser) {
  try {
    if (!paidUser.referredBy || paidUser.firstMembershipPaid) return; // sirf pehli baar
    const referrer = await User.findOne({ referralCode: paidUser.referredBy });
    if (!referrer) return;

    referrer.referralCredits = (referrer.referralCredits || 0) + REFERRAL_MEMBERSHIP_BONUS;
    await referrer.save();

    paidUser.firstMembershipPaid = true;
    await paidUser.save();

    await sendTelegram(
      `🎁 REFERRAL BONUS!\n\n` +
      `👤 ${referrer.name || referrer.email} ko ₹${REFERRAL_MEMBERSHIP_BONUS} mile\n` +
      `📧 Referred user: ${paidUser.email}\n` +
      `💰 Total Referral Credits: ₹${referrer.referralCredits}\n` +
      `🕐 ${new Date().toLocaleString('en-IN', {timeZone:'Asia/Kolkata'})}`
    );
  } catch(e) { console.error('creditMembershipReferral error:', e.message); }
}
...
passport.use(new GoogleStrategy({
  ...
  callbackURL: 'https://ambds.com/auth/google/callback',
  scope: ['profile', 'email'],
...
app.get('/auth/logout', (req, res) => {
  req.logout((err) => {
    ...
    req.session.destroy((err2) => {
      ...
      res.clearCookie('connect.sid', { domain: '.ambds.com' });
      res.redirect('/');
    });
  });
});
...
// ===== MEMBER STATUS =====
// Public — trial link ON/OFF hai ya nahi
// ============================================================
// DEMO TRADING (Virtual paisa, real live prices — koi real withdrawal nahi)
// ============================================================
const DEMO_COINS = [
...
async function fetchLivePrices() {
  const ids = DEMO_COINS.map(c => c.symbol).join(',');
  const response = await makeRequest('GET', 'api.coingecko.com', `/api/v3/simple/price?ids=${ids}&vs_currencies=inr`, null, {});
  return response;
}

app.get('/api/demo/market', async (req, res) => {
  try {
    const prices = await fetchLivePrices();
    const market = DEMO_COINS.map(c => ({ symbol: c.symbol, name: c.name, price: prices[c.symbol]?.inr || 0 }));
    res.json({ success: true, market });
  } catch(e) { res.json({ success: false, message: e.message }); }
});

app.get('/api/demo/portfolio/:userId', async (req, res) => {
  try {
    const user = await User.findById(req.params.userId);
    if (!user) return res.json({ success: false });
    const openTrades = await DemoTrade.find({ userId: req.params.userId, status: 'open' });
    const prices = await fetchLivePrices();

    const positions = openTrades.map(t => {
      const currentPrice = prices[t.symbol]?.inr || t.entryPrice;
      const unrealizedPnl = (currentPrice - t.entryPrice) * t.quantity;
      return {
        id: t._id, symbol: t.symbol, quantity: t.quantity,
        entryPrice: t.entryPrice, currentPrice,
        amountInr: t.amountInr, unrealizedPnl
      };
    });

    res.json({ success: true, demoBalance: user.demoBalance, positions });
  } catch(e) { res.json({ success: false, message: e.message }); }
});

app.post('/api/demo/buy', async (req, res) => {
  try {
    const { userId, symbol, amountInr } = req.body;
    const amt = parseFloat(amountInr);
    if (!amt || amt < 100) return res.json({ success: false, message: 'Minimum ₹100 se trade karo' });
    if (!DEMO_COINS.find(c => c.symbol === symbol)) return res.json({ success: false, message: 'Ye coin available nahi hai' });

    const user = await User.findById(userId);
    if (!user) return res.json({ success: false, message: 'User nahi mila' });
    if (user.demoBalance < amt) return res.json({ success: false, message: 'Demo balance kam hai!' });

    const prices = await fetchLivePrices();
    const currentPrice = prices[symbol]?.inr;
    if (!currentPrice) return res.json({ success: false, message: 'Live price nahi mil paya, dobara try karo' });

    const quantity = amt / currentPrice;
    const trade = new DemoTrade({ userId, symbol, quantity, entryPrice: currentPrice, amountInr: amt, status: 'open' });
    await trade.save();

    user.demoBalance -= amt;
    await user.save();

    res.json({ success: true, message: `${symbol} khareeda gaya!`, demoBalance: user.demoBalance, trade });
  } catch(e) { res.json({ success: false, message: e.message }); }
});

app.post('/api/demo/sell', async (req, res) => {
  try {
    const { userId, tradeId } = req.body;
    const trade = await DemoTrade.findOne({ _id: tradeId, userId, status: 'open' });
    if (!trade) return res.json({ success: false, message: 'Trade nahi mila' });

    const prices = await fetchLivePrices();
    const currentPrice = prices[trade.symbol]?.inr;
    if (!currentPrice) return res.json({ success: false, message: 'Live price nahi mil paya, dobara try karo' });

    const exitValue = trade.quantity * currentPrice;
    const pnl = exitValue - trade.amountInr;
    ...
    const user = await User.findById(userId);
    user.demoBalance += exitValue;
    await user.save();

    res.json({ success: true, message: pnl >= 0 ? `✅ ₹${pnl.toFixed(2)} profit!` : `❌ ₹${Math.abs(pnl).toFixed(2)} loss`, pnl, demoBalance: user.demoBalance });
  } catch(e) { res.json({ success: false, message: e.message }); }
});

app.get('/api/demo/history/:userId', async (req, res) => {
  try {
    const trades = await DemoTrade.find({ userId: req.params.userId, status: 'closed' }).sort({ closedAt: -1 }).limit(30);
...
app.post('/api/demo/reset', async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    if (!user) return res.json({ success: false, message: 'User nahi mila' });
    await DemoTrade.deleteMany({ userId, status: 'open' });
    user.demoBalance = 10000;
    await user.save();
    res.json({ success: true, message: 'Demo balance reset ho gaya — ₹10,000!' });
  } catch(e) { res.json({ success: false, message: e.message }); }
});

app.get('/api/trial-status', async (req, res) => {
  try {
    const s = await Settings.findOne({ key: 'trialLinkActive' });
    res.json({ success: true, active: s ? !!s.value : true }); // default ON
  } catch(e) { res.json({ success: true, active: true }); }
});

// Admin — trial link ON/OFF karo
app.post('/admin/trial-toggle', adminAuth, async (req, res) => {
  try {
...
app.get('/api/member-status/:userId', async (req, res) => {
  try {
    ...
    await ensureReferralCode(user);
    res.json({
      success: true, isActive, expiryDate: user.memberExpiry,
      referralCredits: user.referralCredits || 0,
      referralCode: user.referralCode,
      referralLink: `https://ambds.com/?ref=${user.referralCode}`,
  ...
  try {
    ...
    if (plan === 'membership') {
      ...
      const orderData = {
        ...
        order_meta: {
          return_url: `https://ambds.com/?paid=membership&order_id={order_id}`,
      ...
      if (response.payment_session_id) {
        // Payment record turant banao — warna webhook/verify ise dhoond hi nahi payega
        const memPayment = new Payment({
          userId, plan: 'membership', amount: 10, transactionId: orderId,
          userName: user.name || user.email, userEmail: user.email,
          status: 'pending'
        });
        await memPayment.save();
...
// ✅ MEMBERSHIP VERIFY (safe — Cashfree se seedha order status check karta hai)
// ✅ Referral credits se membership renew karo (agar ₹10 ya usse zyada credits hain, koi payment nahi lagega)
app.post('/api/membership/renew-with-credits', async (req, res) => {
  try {
    const { userId } = req.body;
    const user = await User.findById(userId);
    if (!user) return res.json({ success: false, message: 'User nahi mila' });

    if ((user.referralCredits || 0) < 10) {
      return res.json({ success: false, message: `Sirf ₹${user.referralCredits || 0} credits hain — kam se kam ₹10 chahiye. Aur logo ko refer karo!` });
    }

    user.referralCredits -= 10;
    const currentExpiry = user.memberExpiry && new Date(user.memberExpiry) > new Date() ? new Date(user.memberExpiry) : new Date();
    user.memberExpiry = new Date(currentExpiry.getTime() + 30 * 24 * 60 * 60 * 1000);
    await user.save();

    await sendTelegram(
      `🎁 MEMBERSHIP RENEWED (Referral Credits se)! ✅\n\n` +
      `👤 ${user.name || 'Unknown'}\n📧 ${user.email}\n` +
      `📅 Valid Till: ${user.memberExpiry.toLocaleDateString('en-IN')}\n` +
      `💰 Bache Credits: ₹${user.referralCredits}`
    );

    res.json({ success: true, message: 'Membership 30 din aur badh gayi — bina payment ke!', expiryDate: user.memberExpiry, remainingCredits: user.referralCredits });
  } catch(e) {
    res.json({ success: false, message: e.message });
  }
});
...
app.post('/api/membership/verify', async (req, res) => {
  try {
    ...
    const payment = await Payment.findOne({ transactionId: orderId, plan: 'membership' });
    if (!payment) return res.json({ success: false, message: 'Order nahi mila' });

    if (payment.status === 'completed') {
      const user = await User.findById(userId);
      return res.json({ success: true, message: 'Membership pehle hi active hai!', expiryDate: user?.memberExpiry });
    }
    ...
    console.log(`Membership verify: orderId=${orderId}, status=${orderStatus}`);
    ...
    if (orderStatus === 'PAID') {
      ...
      await creditMembershipReferral(user);
      ...
      res.json({ success: true, message: 'Membership 30 din ke liye activate ho gayi!', expiryDate: expiry });
...
app.post('/api/cashfree/create-order', async (req, res) => {
  try {
    ...
    const orderData = {
      ...
      order_meta: {
        return_url: 'https://ambds.com/payment-success?order_id={order_id}&plan=' + plan,
        notify_url: 'https://yt-sub-exchange.onrender.com/api/cashfree/webhook'
      }
    };
...
app.post('/api/cashfree/webhook', async (req, res) => {
  try {
    // Verify webhook signature — Cashfree ke alawa koi aur is route ko fake call na kar sake
    const signature = req.headers['x-webhook-signature'];
    const timestamp = req.headers['x-webhook-timestamp'];
    if (!signature || !timestamp || !req.rawBody) {
      console.warn('Webhook rejected: signature/timestamp missing');
    ...
    if (paymentStatus === 'SUCCESS') {
      ...
      if (payment.status === 'completed') return res.json({ success: true }); // already processed

      // MEMBERSHIP webhook (backup — agar frontend verify call miss ho jaye)
      if (payment.plan === 'membership') {
      ...
      // Payment fail hui — status ko 'failed' mark karo taaki admin panel mein sahi count dikhe
      const payment = await Payment.findOne({ transactionId: orderId });
...
// ============================================================
// SERVICE LINKS (AMBDS ke saare government service ke real URLs)
// ============================================================
const DEFAULT_SERVICE_LINKS = [
  { key: 'jameen-rasid', name: 'जमीन की रसीद', url: 'https://www.bhulagan.bihar.gov.in/' },
  { key: 'bihar-bhumi', name: 'बिहार भूमि', url: 'https://biharbhumi.bihar.gov.in/' },
  { key: 'jamabandi', name: 'जमाबंदी', url: 'https://biharbhumi.bihar.gov.in/' },
  { key: 'khata-khesra', name: 'खाता / खेसरा', url: 'https://biharbhumi.bihar.gov.in/' },
  { key: 'bhu-naksha', name: 'भू-नक्शा', url: 'https://bhunaksha.bihar.gov.in/' },
  { key: 'dakhil-kharij', name: 'दाखिल-खारिज', url: 'https://biharbhumi.bihar.gov.in/' },
  { key: 'bhumi-record', name: 'भूमि रिकॉर्ड', url: 'https://biharbhumi.bihar.gov.in/' },
  { key: 'dakhil-status', name: 'दाखिल Status', url: 'https://biharbhumi.bihar.gov.in/' },
  { key: 'birth-cert', name: 'जन्म प्रमाण पत्र', url: 'https://serviceonline.bihar.gov.in/' },
  { key: 'death-cert', name: 'मृत्यु प्रमाण पत्र', url: 'https://serviceonline.bihar.gov.in/' },
  { key: 'income-cert', name: 'आय प्रमाण पत्र', url: 'https://serviceonline.bihar.gov.in/' },
  { key: 'caste-cert', name: 'जाति प्रमाण पत्र', url: 'https://serviceonline.bihar.gov.in/' },
  { key: 'residence-cert', name: 'निवास प्रमाण पत्र', url: 'https://serviceonline.bihar.gov.in/' },
  { key: 'ews-cert', name: 'EWS प्रमाण पत्र', url: 'https://serviceonline.bihar.gov.in/' },
  { key: 'character-cert', name: 'चरित्र प्रमाण पत्र', url: 'https://serviceonline.bihar.gov.in/' },
  { key: 'pan-card', name: 'PAN Card', url: 'https://www.onlineservices.nsdl.com/paam/endUserRegisterContact.html' },
  { key: 'voter-id', name: 'Voter ID', url: 'https://voters.eci.gov.in/' },
  { key: 'aadhaar', name: 'Aadhaar', url: 'https://uidai.gov.in/' },
  { key: 'driving-licence', name: 'Driving Licence', url: 'https://parivahan.gov.in/parivahan//en/content/driving-licence' },
  { key: 'passport', name: 'Passport', url: 'https://www.passportindia.gov.in/' },
  { key: 'eshram', name: 'e-Shram Card', url: 'https://eshram.gov.in/' },
  { key: 'labour-card', name: 'Labour Card', url: 'https://labour.bih.nic.in/' },
  { key: 'pm-kisan', name: 'PM-Kisan', url: 'https://pmkisan.gov.in/' },
  { key: 'ration-card', name: 'Ration Card', url: 'https://epds.bihar.gov.in/' },
  { key: 'ayushman-card', name: 'Ayushman Card', url: 'https://beneficiary.nha.gov.in/' },
  { key: 'scholarship', name: 'Scholarship', url: 'https://scholarships.gov.in/' },
  { key: 'pension-status', name: 'Pension Status', url: 'https://sspmis.bihar.gov.in/' },
  { key: 'sarkari-yojana', name: 'सरकारी योजनाएँ', url: 'https://state.bihar.gov.in/' }
];

async function seedServiceLinks() {
  for (const item of DEFAULT_SERVICE_LINKS) {
    const exists = await ServiceLink.findOne({ key: item.key });
    if (!exists) await new ServiceLink(item).save();
  }
}
seedServiceLinks();

// Public — frontend ye call karke saare service links fetch karta hai
// Public — active announcement dikhane ke liye (agar admin ne bheja hai)
app.get('/api/announcement', async (req, res) => {
  try {
    const a = await Announcement.findOne().sort({ updatedAt: -1 });
    if (a && a.active) res.json({ success: true, message: a.message });
    else res.json({ success: true, message: '' });
  } catch(e) { res.json({ success: false, message: '' }); }
});

// Admin — broadcast message bhejo (sab users ko website khulne pe dikhega)
...
// Public — jab user koi service khole (membership ke saath), log karo — Applications tab ke liye
app.post('/api/service-click', async (req, res) => {
  try {
    const { userId, userName, userEmail, serviceKey, serviceName } = req.body;
    const app_ = new Application({ userId, userName, userEmail, serviceKey, serviceName });
    await app_.save();
    res.json({ success: true });
  } catch(e) { res.json({ success: false }); }
});

// Admin — Applications list (real-time — jab bhi call hoga fresh data dega)
app.get('/admin/applications', adminAuth, async (req, res) => {
  try {
    const apps = await Application.find().sort({ clickedAt: -1 }).limit(200);
...
app.get('/api/service-links', async (req, res) => {
  try {
    ...
    res.json({ success: true, links: map });
  } catch(e) { res.json({ success: false, message: e.message }); }
});

// Admin — saare links list karo (edit karne ke liye)
app.get('/admin/service-links', adminAuth, async (req, res) => {
  try {
    const links = await ServiceLink.find().sort({ name: 1 });
    res.json({ success: true, links });
  } catch(e) { res.json({ success: false, message: e.message }); }
});

// Admin — ek link update/add/remove karo (khali URL = "Jald aayega" dikhega)
app.post('/admin/service-links/update', adminAuth, async (req, res) => {
  try {
    const { key, url } = req.body;
    if (!key) return res.json({ success: false, message: 'Key zaroori hai' });
    const link = await ServiceLink.findOneAndUpdate(
      { key },
      { url: url || '', updatedAt: new Date() },
...
// Admin — Payment status breakdown (completed / pending / failed counts)
app.get('/admin/payment-summary', adminAuth, async (req, res) => {
  try {
    const [completed, pending, failed, totalRevenue] = await Promise.all([
...
// ============================================================
// AI AGENT (Real Claude AI — admin ke commands samajhta hai)
// ============================================================
const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY || '';

const AI_TOOLS = [
  { name: 'get_stats', description: 'Total users, active members, aur revenue dikhao', input_schema: { type: 'object', properties: {} } },
  { name: 'get_payment_summary', description: 'Payment status breakdown — completed, pending, failed', input_schema: { type: 'object', properties: {} } },
  { name: 'get_pending_payments', description: 'Pending payments ki list dikhao', input_schema: { type: 'object', properties: {} } },
  { name: 'get_applications', description: 'Recent service applications/clicks dikhao', input_schema: { type: 'object', properties: {} } },
  { name: 'get_trial_status', description: 'Free trial link (जमीन की रसीद) abhi ON hai ya OFF, check karo', input_schema: { type: 'object', properties: {} } },
  { name: 'toggle_trial', description: 'Free trial link ko ON ya OFF karo', input_schema: { type: 'object', properties: { active: { type: 'boolean', description: 'true = ON, false = OFF' } }, required: ['active'] } },
  { name: 'send_broadcast', description: 'Sab users ko ek message dikhao (banner ki tarah website pe)', input_schema: { type: 'object', properties: { message: { type: 'string' } }, required: ['message'] } },
  { name: 'clear_broadcast', description: 'Broadcast message hatao', input_schema: { type: 'object', properties: {} } },
  { name: 'update_service_link', description: 'Kisi service (jaise jamabandi, pan-card) ka real URL update karo', input_schema: { type: 'object', properties: { key: { type: 'string' }, url: { type: 'string' } }, required: ['key', 'url'] } },
  { name: 'list_service_links', description: 'Saare service links aur unka status (live/khali) dikhao', input_schema: { type: 'object', properties: {} } }
];
...
async function executeAiTool(name, input) {
  ...
  switch (name) {
    case 'get_stats': {
      ...
      const activeMembers = await User.countDocuments({ memberExpiry: { $gt: now } });
      const revenueData = await Payment.aggregate([{ $match: { status: 'completed' } }, { $group: { _id: null, total: { $sum: '$amount' } } }]);
      return { totalUsers, activeMembers, revenue: revenueData[0]?.total || 0 };
    }
    ...
    case 'get_pending_payments': {
      ...
      return { payments: payments.map(p => ({ user: p.userName || p.userEmail, amount: p.amount, plan: p.plan })) };
    }
    case 'get_applications': {
      const apps = await Application.find().sort({ clickedAt: -1 }).limit(10);
      return { applications: apps.map(a => ({ user: a.userName || a.userEmail, service: a.serviceKey, time: a.clickedAt })) };
    }
    case 'get_trial_status': {
      const s = await Settings.findOne({ key: 'trialLinkActive' });
      return { active: s ? !!s.value : true };
    }
    case 'toggle_trial': {
      await Settings.findOneAndUpdate({ key: 'trialLinkActive' }, { value: !!input.active }, { upsert: true });
      return { success: true, active: !!input.active };
    }
    case 'send_broadcast': {
      let a = await Announcement.findOne();
      if (!a) a = new Announcement();
      a.message = input.message; a.active = true; a.updatedAt = new Date();
      await a.save();
      return { success: true };
    }
    case 'clear_broadcast': {
      let a = await Announcement.findOne();
      if (a) { a.message = ''; a.active = false; await a.save(); }
      return { success: true };
    }
    case 'update_service_link': {
      const link = await ServiceLink.findOneAndUpdate({ key: input.key }, { url: input.url || '', updatedAt: new Date() }, { new: true, upsert: true });
      return { success: true, key: link.key, url: link.url };
    }
    case 'list_service_links': {
      const links = await ServiceLink.find();
      return { links: links.map(l => ({ key: l.key, name: l.name, live: !!(l.url && l.url.trim()) })) };
    }
    default:
...
app.post('/admin/ai-agent', adminAuth, async (req, res) => {
  try {
    if (!ANTHROPIC_API_KEY) {
      return res.json({ success: false, message: 'ANTHROPIC_API_KEY set nahi hai Render env variables mein.' });
    }
    const { message } = req.body;
    if (!message) return res.json({ success: false, message: 'Message khali hai' });
    ...
    while (loopCount < 5) {
      loopCount++;
      const response = await makeRequest('POST', 'api.anthropic.com', '/v1/messages', {
        model: 'claude-sonnet-4-5',
        max_tokens: 1024,
        system: 'Tum AMBDS website ke admin ka private AI assistant ho. Hindi/Hinglish mein friendly, concise reply do. Jab data chahiye ho, tools use karo. Sirf admin ke diye commands follow karo.',
        tools: AI_TOOLS,
        messages
      }, {
      ...
      if (response.error) {
        return res.json({ success: false, message: 'AI Error: ' + (response.error.message || JSON.stringify(response.error)) });
      }
      ...
      if (toolUseBlocks.length === 0) break; // AI ne final answer de diya

      messages.push({ role: 'assistant', content: response.content });
      const toolResults = [];
      for (const tb of toolUseBlocks) {
        const result = await executeAiTool(tb.name, tb.input || {});
        toolResults.push({ type: 'tool_result', tool_use_id: tb.id, content: JSON.stringify(result) });
      }
      messages.push({ role: 'user', content: toolResults });
    }

    res.json({ success: true, reply: finalText || 'Kaam ho gaya!' });
  } catch(e) {
