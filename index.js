...
const COMMISSION_SLABS = {
  basic:    30,   // ₹200 plan -> ₹30 commission
  silver:   60,   // ₹500 plan -> ₹60 commission
  gold:     120,  // ₹1000 plan -> ₹120 commission
  diamond:  160,  // ₹1500 plan -> ₹160 commission
  platinum: 250   // ₹2500 plan -> ₹250 commission
};
...
app.post('/api/earn/subscribe', async (req, res) => {
  try {
    ...
    // Channel subscribe count badhao
    await EarnChannel.findByIdAndUpdate(channelId, { $inc: { totalSubscribes: 1 } });
...
app.post('/admin/earn-channel/add', adminAuth, async (req, res) => {
  try {
    ...
    const channel = new EarnChannel({ channelUrl, channelName: channelName || '', isActive: true });
    await channel.save();
...
app.get('/admin/earn-stats', adminAuth, async (req, res) => {
  try {
    ...
    res.json({ success: true, totalEarned: totalEarned[0]?.total || 0, totalSubscribes, topEarners });
  } catch(e) { res.json({ success: false }); }
});
