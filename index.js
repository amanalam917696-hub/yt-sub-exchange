...
async function updateSlots() {
  // Expire users whose time is up based on expiresAt
  const now = new Date();
  await User.updateMany(
    { isActive: true, expiresAt: { $lt: now } },
    { $set: { isActive: false } }
  );
  // Also expire based on old ACTIVE_DURATION for users without expiresAt
  ...
  const expiredOld = await User.find({ isActive: true, expiresAt: null, startTime: { $lt: new Date(Date.now() - ACTIVE_DURATION) } });
  for (const u of expiredOld) {
    u.isActive = false;
    u.adsWatched = 0;
    u.subsGiven = 0;
    await u.save();
...
app.post('/api/cashfree/verify', async (req, res) => {
  try {
    ...
    if(orderStatus === 'PAID') {
      ...
      res.json({ success: true, message: 'Payment successful!', plan });
    } else if(orderStatus === 'ACTIVE') {
      // Payment still processing - treat as success
      const user = await User.findById(userId);
      if(user) {
        user.plan = plan;
        user.subsGiven = 0;
      ...
      res.json({ success: true, message: 'Payment processing!', plan });
    } else {
