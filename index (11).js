...
app.get('/api/get-channels/:userId', async (req, res) => {
  try {
    ...
    const MAX_ACTIVE = 10; // Max 10 active at a time
    ...
    // Queue position - count users who paid before this user and are waiting
    const queuePosition = await User.countDocuments({
      _id: { $ne: req.params.userId },
      isActive: false,
      paymentVerified: true,
      plan: { $in: ['basic','silver','gold','platinum'] },
      createdAt: { $lt: user.createdAt }
    });
    
    // Estimate wait time based on queue position
    // Average plan duration ~18 hours, so each slot frees up roughly
    const avgHoursPerSlot = 18;
    const estimatedHours = Math.ceil(queuePosition / MAX_ACTIVE) * avgHoursPerSlot;
    
    res.json({ 
      channels: activeUsers, 
      totalActive: activeCount,
      required: Math.min(activeUsers.length, 10),
...
app.post('/api/auto-activate', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Auto activated!', expiresAt });
  } catch(e) { res.json({ success: false }); }
});
